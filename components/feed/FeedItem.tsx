...
export function FeedItem({ item, index }: FeedItemProps) {
  ...
  const getLevelColors = (level: string) => {
    const colors: { [key: string]: { border: string; bg: string; text: string } } = {
      Federal: { border: "border-l-red-200", bg: "bg-red-50/30", text: "text-red-600" },
      Provincial: { border: "border-l-blue-200", bg: "bg-blue-50/30", text: "text-blue-600" },
      Municipal: { border: "border-l-green-200", bg: "bg-green-50/30", text: "text-green-600" },
    };
    return colors[level] || { border: "border-l-gray-200", bg: "bg-white", text: "text-gray-600" };
  };
        ...
        <div className="flex items-start justify-between mb-3">
          <div className="flex-1">
            <div className="flex items-center gap-2 mb-2">
              ...
              {item.isNew && (
                <span className="px-2 py-0.5 bg-gray-900 text-white text-xs font-medium rounded-full">New</span>
              )}
            </div>
